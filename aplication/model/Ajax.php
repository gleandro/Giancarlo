...
class Ajax{
  ...
  echo '</pre>';
	...
	function actualizarCotizacionAjax____(){
		...
		$descripcion = $_POST['descripcion'];
		foreach ($nombreDia as $key => $nombre) {
			$query2 = new Consulta("INSERT INTO paquetes_itinerario VALUES('','". $id_paquete ."','41','". $nombre['0'] ."','". $descripcion[$key]['0'] ."') ");
	...
	function registrarPaqueteAjax(){
		...
		foreach ($nombreDia as $key => $nombre) {
			...
			$query2 = new Consulta("INSERT INTO paquetes_itinerario VALUES('','". $nuevoid ."','41','". $nombre['0'] ."','". $descripcion[$key]['0'] ."') ");
			$nuevoid2 = $query2->nuevoid();
			$servicioarray = $servicio[$key];
			foreach ($servicioarray as $llave => $value) {
				$query3 = new Consulta("INSERT INTO paquetes_itinerario_detalle VALUES('','". $nuevoid2 ."','". $value ."') ");
